...
abstract: |
  ...
  We connect three big ideas that are common across multiple sports, and explore both the shared similarities and individual idiosyncracies of analytical approaches in each sport. 
  While our focus is on the concepts underlying each type of analysis, any implementation necessarily involves statistical methodologies, computational tools, and sources of data. 
...
The authors introduce a multinomial logistic regression approach, which takes into account the current down, time remaining, yards from endzone, yards to go, and the indicators for goal down situation and whether there are less than two minutes remaining in the half. 
...
In addition, @pelechrinis2019 develops an expected points framework in the same spirit as previous work, but accounts for the strength of the opponents in their method.
...
One of the most notable and well-studied is the use of expected points in the evaluation of 4th down strategy, and there is near universal consensus in the literature that NFL teams have been too conservative when making 4th down decisions. 
...
The author concludes that teams don't go for it enough if they are trying to maximize their probability of winning the game. 
...
Recently, @lopez2020bigger presented an introduction to NFL tracking data, and as an example, they examine 4th down behavior.
Rather than looking at the traditional distance to go, which is rounded, they have the exact position of the ball leading to an exact distance to go.
They demonstrate that teams are still conservative when choosing to go for it or punt on 4th down, but the effect may be overstated. 
...
As for quarterback evaluation, @White2002 present a tiered logistic regression method that can be, in general, applied to any regression setting with a polychotomous response.
...
Accordingly, the model results are utilized to obtain ratings and rankings for NFL passers.

Expected point value is also the main component of a novel quarterback evaluation metric introduced by @Reyers2021quarterback. 
The authors take advantage of player tracking data to account for different passing and running options available to the quarterback.
The expected points and probabilities associated with the available options are estimated using the method of ensemble learning via stacking.

In addition, @Alamar2010 implements an expected points framework to examine play calling in the NFL. 
However, rather than assessing each play on its own, the author evaluates the play in the context of the drive. 
Based on play-by-play data from 2005 through 2008, it is determined that teams are under-utilizing passing plays in some situations.  
...
Specifically, they look at surprise on-sides kicks versus regular kickoff and the decision to accept a touchback versus returning the kickoff. 
...
Table \ref{tab:tied} reveals that the expected number of runs scored in the remainder of the inning is 1.94 runs, while the probability of scoring zero runs is 0.13. 
...
@Lock2014 implement a random forest method to provide a win probability estimate before each play in a football game. 
...
The NFL teams' colors and logos shown in Figure \ref{fig:nflfastr-wp} were provided by the \pkg{teamcolors} package. 
...
Thus, the competition not only sparked interest in sports analytics, but also resulted in peer-reviewed research which, in that case, demonstrated the value of superior data over sophisticated modeling. 
...
This annual competition has similarly fueled new research directions in American football and a [*JQAS* special issue on player tracking data in the National Football League (NFL)](https://www.degruyter.com/journal/key/jqas/16/2/html) of [@lopez2020bigger].
...
Similar competitions that provide opportunities for aspiring researchers to tackle sports analytics problems include the [Big Data Cup](https://www.stathletes.com/big-data-cup/) for ice hockey and the [SABR Diamond Dollars Case Competition](https://sabr.org/analytics/case) for baseball.
