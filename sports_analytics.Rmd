...
For example, in American football, $s$ is determined by factors such as down, yardage to the next first down, and field position. 
...
Due to limitations regarding the amount of data collected, the football field is broken into 10-yard strips, centered at their midpoints (e.g. 5, 15, 25, 35, etc.), and the expected points are calculated for each of those field positions. 
Early work on expected point values in football also includes @Carroll1988hidden, which popularizing the idea of play evaluation using expected points and expected points added.

@goldner2012markov and @goldner2017situational proposes a Markov model for estimating expected points in football. 
...
The expected points for a play can then be calculated accordingly, by taking the sum of the products of the scoring event point values and their associated probabilities.
