...
A demonstration of the EPV model presented in @cervone2016multiresolution's paper is available on the first author's GitHub at <https://github.com/dcervone/EPVDemo>. As an illustration, using the provided tracking data, Figure \ref{fig:epv-demo} displays a snapshot of a possession during the 2013-14 NBA regular season matchup between the Miami Heat and the Brooklyn Nets on November 1, 2013. Miami is the team on offense in this play, and the possession outcome is a 26-foot three-point miss by Mario Chalmers of the Heat. The plot consists of two elements: 1) (bottom) the player locations on the court at a particular moment in this possession--when the ball just left Chalmers' hands, and 2) (top) a line graph showing how the EPV is continuously changing throughout the play until the three-point attempt. For this entire possession, the estimated EPV for the Miami Heat reaches its peak at $1.276$ at this specific time point.
...
```{r epv-demo, out.width="100%", fig.cap="Player locations and estimated EPV for a possession during Miami Heat (red) vs. Brooklyn Nets (black) on November 1, 2013. The captured moment is when Miami's Mario Chalmers just releases a three-point shot, which ends up being a miss."}
