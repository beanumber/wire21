...
A demonstration of the EPV model presented in @cervone2016multiresolution's paper is available on the first author's GitHub at <https://github.com/dcervone/EPVDemo>. 
As an illustration, using the provided tracking data, Figure \ref{fig:epv-demo} displays a snapshot of a possession during the 2013-14 NBA regular season matchup between the Miami Heat and the Brooklyn Nets on November 1, 2013. Miami is the team on offense in this play, and the possession outcome is a 26-foot three-point miss by Mario Chalmers of the Heat. 
...
and 2) (top) a line graph showing how the EPV is continuously changing throughout the play until the three-point attempt. For this entire possession, the estimated EPV for the Miami Heat reaches its peak at $1.276$ at this specific time point.
...
```{r epv-demo, out.width="100%", fig.cap="Player locations and estimated EPV for a possession during the Miami Heat (red) vs. Brooklyn Nets (black) NBA game on November 1, 2013. The captured moment is when Miami's Mario Chalmers just releases a three-point shot, which ends up being a miss."}
...
In this paper, the authors propose a Bayesian linear regression model to assess the impact of individual players on their team's win probability at any given time of a basketball game. 
...
<!-- I think we should also mention the sportsdataverse (https://github.com/sportsdataverse, https://sportsdataverse.org) 

maybe some kaggle competitions (march madness, cite mike and greg's paper; and big data bowl)
-->
...
# we could plot greg's chess moves
# doesn't work for me... python/reticulate issues
# library(chessR)
# greg <- get_raw_chessdotcom(usernames = "fatgreggy",
#                             year_month = "202206")
# m <- extract_moves_as_game(greg[1, ])
# plot_moves(m)
```
