...
In recent years, a number of statistical methods have been used to build well-calibrated win probability models in American football. @Lock2014 used random forests as a means for estimating win probability before each play in a football game. In addition, @Yurko2019 estimated win probability in the NFL using a generalized additive model, as part of the `nflscrapr` package and nflwar framework. A major highlight of this framework is it is fully reproducible and uses publicly available data. One of @Yurko2019's goals was also to encourage researchers to "use, explore, and improve upon our work", which ultimately inspired \pkg{nflfastR}, known as the successor to `nflscrapr`

(add a wp example from nflfastR)
