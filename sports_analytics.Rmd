...
tibble(
  ...
  kable(booktabs = TRUE,
        linesep = "",
        caption = "A summary of sport-specific packages available on the Comprehensive R Archive Network (CRAN). \\label{tab:ctv}") |> 
...
Sharing a similar goal with \pkg{teamcolors} is \pkg{nflplotR} [@R-nflplotR], which also provides \pkg{ggplot2} extensions but is designed specifically for the NFL.
A great feature of \pkg{nflplotR} is the \pkg{ggplot2} "geom" (geometric object) functions that enhance high-quality plotting of NFL team logos and player images.
...
The logos of all 32 American football clubs are plotted in place of the usual dots, giving a better description of which team each data point represents.

```{r nflplotr, fig.cap="Offensive and defensive per play for the 2021 NFL regular season, plotted with \\pkg{nflplotR} using data from \\pkg{nflfastR}."}
