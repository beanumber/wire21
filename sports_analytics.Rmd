...
In basketball, @cervone2014pointwise and @cervone2016multiresolution introduces expected possession value (EPV) as an assessment of a player's on-court performance. This metric is a continuous-time estimate of the expected number of points for the offensive team on a given possession using player and ball locations. The EPV takes into account all possible outcomes (a shot attempt, a pass, etc.) for a given player with the ball, with different weights being assigned to each decision. See also @bornn2017studying for more information on how tracking data have enabled advanced statistical analyses of basketball in recent years.

more basketball @Sicilia2019DeepHoops

@Yurko2020going uses tracking data provided by the 2019 NFL Big Data Bowl (citation) to model the expected yards gained for a ball-carrier during the course of a play. As opposed to only giving a single estimate for the expected yards gained, the authors use conditional density estimation to obtain a probability distribution for the number of yards gained during the play. 
