...
abstract: |
  ...
  In this paper, we connect three big ideas that are common across multiple sports, and explore both the shared similarities and individual idiosyncracies of analytical approaches in each sport. 
...
In particular, the author considers a football drive as an *absorbing Markov chain*, consisting of distinct *absorbing states* that include touchdowns, field goals, and other play outcomes. 
...
The concept of expected points in American football has many applications.  One of the most notable and well-studied topic is the evaluation of 4th down strategy.
...
In the past, when looking at down and distance data to study whether NFL coaches are making good decisions about whether to "got for it" or punt on 4th down, the distance data is only a rounded approximation of the true distance "to go" (i.e. 1 yard, 1 foot, and 1 inch will all be recorded as 4th and 1. 
...
Many past analyses of the decision to go for it or not on 4th down conclude that coached in the NFL are too conservative in their decisions making. 
@lopez2020bigger also concludes that coaches are too conservative on 4th down decision making, but the effect may be overstated due to the way in which to go yardage has been recorded in the past.  
...
@kumagai2021hockey offer an EPV metric for ice hockey. 
...
First, we briefly introduce a simple method for estimating team strength based on won-loss record. 
...
The impetus for all methods for estimating team strength is the realization that won-loss records are a noisy measure of team strength. 
...
The first method for estimating team strength to become popular in sports analytics is a measure of expected winning percentage---often called Pythagorean expectation---developed by @james2003new. 
...
$$
  \frac{X^\alpha}{X^\alpha + Y^\alpha} = \frac{1}{1 + (Y/X)^2}
$$
and fitting a non-linear model (see similar discussion in @baumer2021mdsr). 
Figure \ref{fig:pythag} illustrates the quality of the fit in Major League Baseball since 1954. 
...
data_space +
  stat_function(fun = exp_wpct, args = list(2), size = 2, color = "navy") +
  stat_function(fun = exp_wpct, args = list(optimal), size = 2, color = "gold")
```
...
For those that have never reached a rating of 2400 or better, $K=20$ is assigned. 
Finally, $K=40$ is given to new players with under 30 games played, and players under the age of 18 with rating fewer than 2300 [@fide2022rating].
...
@koopman2015dynamic uses a Bayesian state-space model to develop team ratings for English Premier League soccer that improves on bookmaker's odds. 
...
However, a growing body of literature is informed by non-performance data about sports. 
This research includes fields like sports economics and kinesiology that are subfields of mature disciplines (a treatment of either is well beyond the scope of the this paper).
In addition to this body of research, we note that today, professional sports teams are collecting biometric data on their own players, and presumably using that data to learn about how their workouts, sleep patterns, and diets impact their athletic performance. 
While this research may constitute "emerging methodologies," it will likely never be published, because each team will use it to their competitive advantage. 

Nevertheless, in this Section we discuss concepts stemming from one notable, relatively new source of (semi-)public non-performance sports data: data from betting markets.
...
The increasing interest in sports gambling has led to increasing interest in sports gambling *data*, and that data has proven useful to researchers in two major ways. 
...
## Sports betting market data {#sec:betting-market-data}

While @Lopez2018 uses betting market data to model team strengths, it does not directly address strategies for betting or inefficiencies in betting markets. Early work by @gandar1988testing examines 
Related work such as @lacey1990estimation and @Boulier2006 both explore the efficiency of NFL betting markets in the mid-1980s and late-1990s, respectively.  Neither paper finds strong evidence for inefficiencies in the markets.  @boulier2003predicting compared the predictive performance of power rankings and media experts to the betting market for NFL games and found that the betting market was the best for predicting winners. More recently @Buttrey2016 explored the NHL betting market. They produced a model to predict win probabilities in given games, and games where the predicted probability differed from the market price bets were placed.  They find that using their methods, that they were able to produce a positive return on investment.  

@Boulier2006 that there is insufficient evidence to conclude that betting markets were inefficient from 1994--2000.
@boulier2003predicting test several predictors for NFL games and find that the betting market is the best predictor.
@Buttrey2016 demonstrate a method that produces a positive ROI.
@gandar1988testing conclude that stats test are too weak to reject the hypothesis of rationality.  But they do reject rationalist with ``economic tests". 
@lacey1990estimation : Finds the market to be efficient, though they did find some profitable betting rules over the period of time between 1984 and 1986

## Analytics for sports betting pools {#sec:pools}

Another popular type of betting pool is a survivor pool.
@Bergman2017SurvivingAN presents formal optimization approaches for NFL survivor pools and concludes that planning for only part of the season yields optimal results in terms of maximizing survival probability.
...
@imbrogno2022computing
Description: The average number of winning survivor pool entries is computed for the 2019 National Football League (NFL) season. The probability of having to share the winning pot, given realistic expectations on game outcomes, is also shown.

@nichols2014impact conclude that the impact of travel is not completely incorporated into the betting markets, but that any effect is too small to find any profitable advantage.  

@PaulWeinbach2014 Market efficiency and behavioral biases in the wnba betting market

@soebbing2013gamblers

@spann2009sports NBA bettors believe that tanking is occurring.  
...
@casals2022systematic offers a systematic review of sport-related packages on CRAN.
...
tibble(
  ...
  kable(
    booktabs = TRUE,
    linesep = "",
    caption = "A summary of sport-specific packages available on the Comprehensive R Archive Network (CRAN). While the major North American sports dominate the list, perhaps the fastest-growing collection is for esports. \\label{tab:ctv}"
  ) |> 
...
We note how the contextual information provided by the chessboard is instrumental in helping the reader contextualize the data. 
...
```{r chess, eval=whoami::username() == 'bsbaumer', fig.show="hold", out.width="49%", fig.cap="At left, the starting chess board printed via the \\pkg{chess} package. At right, the final position for one of the authors' recent wins (a checkmate playing Black)."}
library(chess)
...
Consider, for example, how much time the surfaces plotted in Figure \ref{fig:sportyR} would save you, and how helpful they would be in contextualizing player tracking data (such as those shown in Figure \ref{fig:epv-demo}).
...
For example, before George Lindsay's work (see Section \ref{sec:ev}), most of the baseball data that was publicly available was seasonal: it showed only season totals for each player. 
...
It's West Coast counterpart is [The Cascadia Symposium on Statistics in Sports](http://www.cascadiasports.com/). 
