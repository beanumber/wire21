...
For example, although belonging to the same bat-and-ball family, baseball and cricket differ in strategies such as batting order or sacrifice plays.
...
Many applications can be derived from this framework, including predicting which footballer is most likely to receive the next pass from the current on-ball player.
...
Covariates included in this win probability model are the current down, score differential, time remaining, adjusted score, point spread, number of timeouts remaining for each team, total points scored, current yard line, and yards to go for a first down. 
