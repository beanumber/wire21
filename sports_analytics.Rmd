...
He concludes that teams don't go for it enough if they are trying to maximize their probability of winning the game. 
...
Recently, @lopez2020bigger presents an introduction to NFL tracking data, and as an example, examines 4th down behavior.
...
They demonstrate that teams are still conservative when choosing to go for it or punting on 4th down, but the effect may be overstated. 
...
For quarterback evaluation, @White2002 present a tiered logistic regression method that can be, in general, applied to any regression setting with a polychotomous response.
...
Based on play-by-play data from 2005 through 2008, he determines that teams are under-utilizing passing plays in some situations.  
...
The strategy of maximizing expected points in basketball has lead directly to the proliferation of three-point shooting in the NBA. 

In American football, @Yurko2020going use tracking data provided by the [2019 NFL Big Data Bowl](https://operations.nfl.com/gameday/analytics/big-data-bowl/past-big-data-bowl-recaps/) (see Section \ref{sec:opportunities}) to model the expected yards gained for a ball-carrier during the course of a play. 
...
Table \ref{tab:tied} (derived from Table \ref{tab:erm}) reveals that the expected number of runs scored in the remainder of the inning is 1.94 runs, while the probability of scoring zero runs is 0.13. 
...
```{r chess, eval=whoami::username() == 'bbsdaumer', fig.show="hold", out.width="49%", fig.cap="At left, the starting chess board printed via the \\pkg{chess} package. At right, the final position for one of the authors' recent wins (a checkmate playing Black)."}
library(chess)
