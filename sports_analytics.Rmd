...
In addition, @Yurko2019 estimated win probability in the NFL using a generalized additive model (GAM), as part of the `nflscrapr` package and nflWAR framework. This model takes into account the estimated expected points obtained from the model described in \ref{sec:ev}, along with other predictors for time, current half, and timeouts. A major highlight of this framework is it is fully reproducible and uses publicly available data. One of @Yurko2019's goals was also to encourage researchers to "use, explore, and improve upon our work", which ultimately inspired \pkg{nflfastR}, known as the successor to `nflscrapr`.

These two frameworks were also implemented in @Yam2019 with minimal modifications. Specifically, the authors combined both @Lock2014 and @Yurko2019's approaches to estimate the win probability for each play, with an overall goal of assessing fourth down decision making in gridiron football.

Figure \ref{fig:nflfastr_wp}
...
```{r nflfastr_wp, fig.cap="WP for MIN vs NO (add more later)"}
library(nflfastR)
load_pbp(2017) |> 
...
Examples of win probability can also be found in other sports, with a diverse range of statistical techniques being used to estimate the probability of winning for a player or team.

@Brenzel2019 used a three-dimensional Markov model to estimate win probability in curling. 

In esport, @Maymin2020 proposed a well-calibrated in-game win probability model for each specific moment during a game of League of Legends.

More recently, @Guan2022 developed an in-game win probability model for the National Rugby League using a functional data analysis approach. 
