...
This sophisticated type of data allows researchers to produce advanced analyses of the captured spatio-temporal information and better understand the game, which is much more difficult to be accomplished with pre-existing resources such as traditional box-score results and play-by-play data.

In basketball, @cervone2014pointwise and @cervone2016multiresolution introduce expected possession value (EPV) as an assessment of a player's on-court performance. 
...
Another framework for estimating expected points in basketball was proposed by @Sicilia2019DeepHoops. 
The authors first consider a classification model which returns the probabilities for whether a player would commit a foul (shooting and non-shooting), turnover, or attempt a shot. 
The values associated with each of those "terminal actions" are then utilized to compute the expected points within a basketball play.
...
In American football, @Yurko2020going use tracking data provided by the 2019 NFL Big Data Bowl (citation) to model the expected yards gained for a ball-carrier during the course of a play. 
...
Accordingly, the probability of various types of outcome at the end of a play such as a touchdown or a first-down gain can be computed from the yards gained distribution.

The notion of EPV has also been extended to soccer recently.
@fernandez2021framework uses deep learning methods to examine the instantaneous expected value of soccer possessions. 
