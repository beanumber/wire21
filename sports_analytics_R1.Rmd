...
While the sources of sports data are too numerous to list, we discuss the use of non-performance data (most notably, sports market betting data) in Section \ref{sec:betting}, and in Section \ref{sec:tools} we highlight a few computational tools (including a table of R packages) that make this kind of work possible. 
...
# Non-performance data {#sec:betting}
...
```{r chess, eval=whoami::username() == 'bbaumer', fig.show="hold", out.width="49%", fig.cap="At left, the starting chess board printed via the \\pkg{chess} package. At right, the final position for one of the authors' recent wins (a checkmate playing Black)."}
library(chess)
